import logging from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update from telegram.ext import ApplicationBuilder, CommandHandler, CallbackQueryHandler, ContextTypes from datetime import datetime, timedelta
...
def get_main_menu(): keyboard = [ [InlineKeyboardButton("Balance💰", callback_data='balance')], [InlineKeyboardButton("My Profile👤", callback_data='profile')], [InlineKeyboardButton("Referral Link👥", callback_data='referral')], [InlineKeyboardButton("NFT Withdrawal📤", callback_data='withdraw')], [InlineKeyboardButton("Deposit📥", callback_data='deposit')], [InlineKeyboardButton("Daily bonus🎁", callback_data='bonus')], ] return InlineKeyboardMarkup(keyboard)

ارسال منو

async def send_main_menu(chat_id, context): await context.bot.send_message( chat_id=chat_id, text="👋 Welcome back to the main menu.\nPlease select one of the options below:", reply_markup=get_main_menu() )

دستور شروع بات

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user user_id = user.id referrer_id = None
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = query.from_user.id user_data = users.get(user_id)
...
if query.data == 'balance':
    await query.edit_message_text(
        f"💰 Your balance: {user_data['balance']:.2f} TON",
    ...
    if len(user_data["referrals"]) < 10:
        await query.edit_message_text(
            "❌ You need to invite at least 10 users to withdraw NFTs.",
            reply_markup=back_button()
        )
    else:
        keyboard = [
    ...
    if user_data["balance"] >= price:
        ...
        await query.edit_message_text("❌ Insufficient balance for this NFT.", reply_markup=back_button())

elif query.data == 'deposit':
    await query.edit_message_text("💼 Deposit feature is coming soon...", reply_markup=back_button())
    ...
    if now - user_data["last_bonus"] >= timedelta(hours=24):
        ...
        await query.edit_message_text("🎁 Daily bonus claimed! You received 0.5 TON.", reply_markup=back_button())
    else:
        remaining = timedelta(hours=24) - (now - user_data["last_bonus"])
        await query.edit_message_text(
            f"⏳ You can claim your bonus again in {remaining.seconds//3600}h {(remaining.seconds//60)%60}m.",
            reply_markup=back_button()
        )
...
if name == "main": logging.basicConfig(level=logging.INFO) app = ApplicationBuilder().token(TOKEN).build() app.add_handler(CommandHandler("start", start)) app.add_handler(CallbackQueryHandler(button_handler)) app.run_polling()
